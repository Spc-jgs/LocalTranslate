...
这个 App 的资源特征是双态的，用单一的「占用上限」管不住，约束按两态分别成立。

**空闲态**（没有任何功能被激活）不持有：定时器、轮询循环、子进程、已加载的
模型、打开的采集设备、目录监视。菜单栏常驻本身不算激活。

**激活态**（字幕在跑、扫描在跑、翻译在途）允许重，但每一项重活都必须：

- 带预算——字节数或时长上限，超出即让位，例如 `UsageScanExecutor` 的
  128 MB / 5 秒；
- 带取消路径——会话身份（`lifecycleGeneration` / `sessionID`）能让旧回调
  安静退场；
- 以资源实际释放作为退出的完成条件，而不是隐藏窗口或清空数组。

新增任何常驻物之前先问它属于哪一态。一个在空闲态跑的 `Timer` 就破坏了
第一条，而它不会被任何测试或构建发现。

参考基线（2026-09-02，本机实测，Xcode Release 构建）：

| 指标 | 值 |
| --- | --- |
| 空闲 CPU | 0.0% |
| 空闲内存 | `footprint` 90 MB（`ps` 的 RSS 含共享库，不是有效指标） |
| 翻译与实时字幕写盘 | 零 |
| lifetime 聚合（23,190 事件） | 10 ms 冷 / 0 ms 热 |

SSD 写入量不构成硬件风险：最坏情况（用量页整天开着）约 7 GB/年，
相对现代 SSD 数百 TB 的 TBW 可忽略。不要为此牺牲可读性做写入优化，
但也不要引入无谓的重复写——同一份快照在一轮刷新里被重写多次属于缺陷。

翻译是主功能，它的路径要求最严：全程不写盘、不建索引、不留后台任务。
`keep-alive` 决定 Ollama 侧的模型常驻内存，属于用户可见的取舍，
默认 10 分钟，改动需要说明理由。
