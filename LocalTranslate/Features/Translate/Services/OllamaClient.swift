...
struct OllamaChatRequest: Encodable {
    ...
    /// Modelfile 往往为对话调参（`qwen3.5:4b` 写的是 `temperature 1`），不覆盖就会继承过来。实测同一段技术文本连翻三次会得到三种译文，其中出现英文残留、擅自加括号解释、markdown 标记泄漏。
...
enum OllamaClientError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .modelNotFound(let model):
            ...
            return OllamaFailure.modelNotInstalledMessage(model: model)
...
@MainActor final class OllamaClient {
    ...
    func translateStream(
        _ text: String, style: TranslationStyle, customPrompt: String,
        onPartialResult: @escaping @MainActor @Sendable (String) -> Void
    ) async throws -> String {
        let url = try makeURL(path: "/api/chat")

        let direction = resolveDirection(text)

        let finalSystemPrompt = makeTranslationSystemPrompt(
            direction: direction, style: style, customPrompt: customPrompt)

        // 固定到局部量：404 要报的是这次真正请求的模型，而不是显示报错时设置里恰好写着的那个。
        let requestedModel = AppSettings.model
        ...
        try Self.validate(response, requestedModel: requestedModel)
        ...
        for try await line in bytes.lines {
            ...
            let chunk = try JSONDecoder().decode(
                OllamaStreamResponse.self, from: data)
        ...
        return completeText.trimmingCharacters(in: .whitespacesAndNewlines)
    }

    // MARK: - Prompt Composition

    private func makeTranslationSystemPrompt(
        direction: TranslationDirection, style: TranslationStyle,
        customPrompt: String
    ) -> String {
        ...
        case .custom:
            ...
            let trimmedCustomPrompt = customPrompt.trimmingCharacters(
                in: .whitespacesAndNewlines)

            // 自定义为空时，效果等同默认翻译。
            guard !trimmedCustomPrompt.isEmpty else {
                return basePrompt
            }
        ...
        default:
            ...
            guard let instruction = style.promptInstruction else {
                return basePrompt
            }
    ...
    func installedModels() async throws -> [OllamaInstalledModel] {
        let url = try makeURL(path: "/api/tags")

        let request = URLRequest(url: url, timeoutInterval: 10)

        let (data, response) = try await URLSession.shared.data(for: request)

        try Self.validate(response, requestedModel: nil)
    ...
    func modelDiagnostics(for modelName: String) async throws
        -> OllamaModelDiagnostics
    {
        let nativeContext = try await nativeContextLength(for: modelName)

        let runningModels = try await runningModels()

        let runningModel = runningModels.first { item in
        ...
        return OllamaModelDiagnostics(
            ...
            nativeContextLength: nativeContext,

            runtimeContextLength: runningModel?.contextLength,

            runtimeMemoryBytes: runningModel?.sizeVRAM,

            isRunning: runningModel != nil,

            kvCacheQuantization: nil)
    }

    // MARK: - Native Context

    private func nativeContextLength(for modelName: String) async throws -> Int?
    {
        let url = try makeURL(path: "/api/show")

        let body = OllamaShowRequest(model: modelName, verbose: false)
        ...
        try Self.validate(response, requestedModel: modelName)
        ...
        return extractNativeContextLength(from: modelInfo)
    }

    private func extractNativeContextLength(from modelInfo: [String: Any])
        -> Int?
    {
        if let architecture = modelInfo["general.architecture"] as? String {
            let key = "\(architecture).context_length"

            if let value = integerValue(modelInfo[key]) {
                return value
            }
        }

        let candidates = modelInfo.compactMap { key, value -> (String, Int)? in

            guard key.hasSuffix(".context_length") else {
                return nil
            }

            if key.contains(".vision.") || key.contains(".audio.") {
                return nil
            }

            guard let context = integerValue(value) else {
                return nil
            }
        ...
        return candidates.sorted { $0.0.count < $1.0.count }.first?.1
    }
    ...
    private func runningModels() async throws -> [OllamaRunningModel] {
        let url = try makeURL(path: "/api/ps")
        ...
        let result = try JSONDecoder().decode(OllamaPSResponse.self, from: data)
    ...
    /// 解析本次翻译的语言对。
    ///
    /// 原文已经是目标语言时翻到 `counterpart`：选中文的人选中一段中文，要的是英文，而不是把中文再润色一遍。
    private func resolveDirection(_ text: String) -> TranslationDirection {
        let target = AppSettings.targetLanguage

        let cleanedText = text.trimmingCharacters(in: .whitespacesAndNewlines)

        guard let detected = detectLanguage(cleanedText) else {
            // 认不出来就不假装知道，只固定目标语言。
            return TranslationDirection(
                sourceName: nil, targetName: target.displayName)
        }

        if TranslationLanguage.matching(detected) == target {
            let counterpart = target.counterpart

            return TranslationDirection(
                sourceName: target.displayName,
                targetName: counterpart.displayName)
        }

        return TranslationDirection(
            sourceName: TranslationLanguage.sourceName(for: detected),
    ...
    private func detectLanguage(_ text: String) -> NLLanguage? {
        ...
        if containsMeaningfulChinese(text) {
    ...
    private func containsMeaningfulChinese(_ text: String) -> Bool {
        ...
        let meaningfulCount = text.filter { character in
        ...
        return Double(chineseCount) / Double(meaningfulCount) >= 0.15
    }
    ...
    /// 带模型的请求返回 404 的唯一常见原因；`/api/tags`、`/api/ps` 不带模型，它们的 404 只是普通的 HTTP 失败。
    private static func validate(
        _ response: URLResponse, requestedModel: String?
    ) throws {
        guard let httpResponse = response as? HTTPURLResponse else {
            throw OllamaClientError.invalidResponse(statusCode: -1)
        }
        ...
        if statusCode == 404, let requestedModel {
            throw OllamaClientError.modelNotFound(model: requestedModel)
        }

        throw OllamaClientError.invalidResponse(statusCode: statusCode)
    }

    private func makeURL(path: String) throws -> URL {
        do { return try OllamaEndpoint.url(path: path) } catch {
